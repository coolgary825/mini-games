...
const CACHE = 'brainrot-v3d-2';
const CORE = ['./', './index.html', './manifest.json', './icon-192.png', './icon-512.png', './apple-touch-icon.png', './3d.html', '../arcade3d/app.js', '../arcade3d/world.js', '../arcade3d/rules.js', '../arcade3d/style.css', '../arcade3d/games/brainrot.js', '../chess/vendor/three/three.module.js', '../chess/vendor/three/three.core.js', '../chess/vendor/three/OrbitControls.js'];

self.addEventListener('install', e => { e.waitUntil(caches.open(CACHE).then(c => c.addAll(CORE)).then(() => self.skipWaiting())); });
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(ks => Promise.all(ks.filter(k => k.startsWith('brainrot-') && k !== CACHE).map(k => caches.delete(k)))).then(() => self.clients.claim())); });
