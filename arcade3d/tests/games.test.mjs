...
test('defense taps near a pad select it, open the build panel and close it after building',()=>{const h=harness(Defense);try{const g=h.game;let open=null;h.ui.panel=v=>open=v;const pad=g.pads[3];g.pick(null,{x:pad.x+1.8,z:pad.z});assert.equal(g.selected,pad);assert.equal(open,true);assert(g.range);assert(g.build(0));assert.equal(open,false);g.pick(null,{x:30,z:30});assert.equal(g.selected,null);assert.equal(open,false);}finally{h.dispose();}});
